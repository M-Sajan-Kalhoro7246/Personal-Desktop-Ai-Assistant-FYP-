...
if __name__=="__main__":
    while 1:
        ...
        if NQ in ["write code for ","optimize this code","write code for this","optimise this code","optimise this code"]:
        ...
        elif "jarvis create powerpoint presentation" in QL or "powerpoint"in QL or "create presentation" in QL or "s create presentation " in QL and NQ.split(" ")[0].lower()=="create":
            path=get_bot_response(Q)
            startfile(fr"{getcwd()}\{path}")
            Speak("done sir")
            Speak(random.choice(GoodMsg))
        
        elif "off the pc" in QL or "shutdown laptop" in QL or "shutdown" in QL or "jarvis off pc" in QL or "jarvis shutshow pc" in QL or "jarvis off laptop" in QL:
            
            Speak('System shutdhowing process Started.')
            os.system("shutdown /s /t 1")   
            
        elif "jarvis restart pc" in QL or "restart laptop" in QL or "restart" in QL or "restart pc" in QL:
            
            Speak('System Restarting process Started.')
            os.system("shutdown /r /t 0")
        
        elif "jarvis system" in QL or "system" in QL or "system usage" in QL or "battery" in QL or "information of battery" in QL or "about battery" in QL:
            life = str(psutil.cpu_percent())
            battery = psutil.sensors_battery().percent
            Speak("CPU is at "+life)
            Speak(f"Battery is at ",battery)

        elif "visit" in QL or "start" in QL:
            NameOfTheApp = QL.replace("start", " ")
            NameOfTheApp = NameOfTheApp.replace("open", " ")
            NameOfTheApp = NameOfTheApp.replace("visit", " ")
            pyautogui.press('win')
            time.sleep(1) 
            ...
            time.sleep(0.5)
            Speak(f'Launching a {NameOfTheApp} Sir')
        ...
        elif "read data " in QL or"jarvis read data from my clipboard" in QL or "jarvis read my clipboard" in QL or "read clipboard" in QL or "copy data from my clipboard" in QL:
            QL = QL.replace("jarvis read data from my clipboard", "")
            QL = QL.replace("jarvis read my clipboard", "")
            QL = QL.replace("read data", "")
        ...
        elif ("sunrise" in NQ or "summarise" in NQ or "summarize" in NQ or "transcribe" in NQ or "translate" in NQ or "summarize video" in NQ or "summarise video" ) and "video" in NQ and LQ<10:
            ...
            if GUIWINDOW:GUI["gpt"]=False
            if code!=None:
                if "from Genration_Of_Images import" in code or "import" not in code or "from toolkit.Alarm import set_alarm"in code:
                    exec(code)
